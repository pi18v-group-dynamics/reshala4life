...
namespace Reshala
{
    public partial class Gaus : Form
    {
        ...
        private void Gaus_Load(object sender, EventArgs e)
        {
            ...
            comboBox_Row.SelectedItem = comboBox_Row.Items[0];
        ...
        private void comboBox_Row_SelectedValueChanged(object sender, EventArgs e)
        {
            if (dataGridView1.Rows.Count - 1 > int.Parse(comboBox_Row.SelectedItem.ToString()))//если к примеру было 8 переменных, а стало 2
            {
                dataGridView1.Rows.Clear(); //удаляю коллекцию рядов
                dataGridView1.Rows.Add(int.Parse(comboBox_Row.SelectedItem.ToString())); //добавляю указанное количество строк
            }
            else   //если к примеру было 2 переменных, а стало 5, то добавляю столбцы начиная со 2-го
            {
                dataGridView1.Rows.Add(Math.Abs(int.Parse(comboBox_Row.SelectedItem.ToString()) - dataGridView1.Rows.Count)); //добавляю указанное количество строк
            }
        }
